...
void solve(){
	while(1){
		...
		cin>>n>>m;
		if(n == 0 && m == 0) break;
		if(n == 1 || m == 1) cout<<n * m<<" knights may be placed on a "<<n<<" row "<<m<<" column board.";
		else if((n == 2 || m == 2) && n != 0 && m != 0) cout<<max(n,m) / 4 * 4 + (max(n,m) % 4 == 1? 2 : (max(n, m) % 4 >= 2? 4 : 0))<<" knights may be placed on a "<<n<<" row "<<m<<" column board.";
		else cout<<(m*n + 1) / 2<<" knights may be placed on a "<<n<<" row "<<m<<" column board.";
		cout<<"\n";
	}
}
